...
type Props = {
  ...
  renderImageViewer?: (props: ViewerProps) => React.ReactNode;
  renderVideoViewer?: (props: ViewerProps) => React.ReactNode;
};
...
const FileViewer = ({
  ...
  const fileViewer = useIIFE(() => {
    switch (fileType) {
      case 'image': {
        if (renderImageViewer) {
          return renderImageViewer({ uri: fileMessage.url, onLoadEnd: () => setLoading(false) });
        } else {
          return <ImageViewer uri={fileMessage.url} onLoadEnd={() => setLoading(false)} />;
        }
      }

      case 'video':
      case 'audio': {
        if (renderVideoViewer) {
          return renderVideoViewer({ uri: fileMessage.url, onLoadEnd: () => setLoading(false) });
        } else {
          return (
